...
function createItinerary() {
  ...
  const geocodingPromises = tripData.selected_destinations
    ...
    .map(dest => {
      ...
      return fetch(`http://localhost:5000/api/geocode?q=${encodeURIComponent(searchQuery)}`)
        .then(response => response.json())
        .then(data => {
  ...
  Promise.all(geocodingPromises)
    .then(() => {
      ...
      return fetch(`http://localhost:5000/api/create_trip?_=${timestamp}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
...
function saveTrip(trip) {
  ...
  const tripToSave = {
    ...
    travel_dates: tripData.travel_dates || 'To be determined',
    trip_summary: {
  ...
  setTimeout(() => {
    ...
    if (createTicketBtn && emailInput && ticketResult) {
      ...
      newCreateTicketBtn.addEventListener('click', () => {
        ...
        fetch(`http://localhost:5000/api/create_ticket?_=${timestamp}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Cache-Control': 'no-cache, no-store, must-revalidate',
